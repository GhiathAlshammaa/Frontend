...
export class GapiService {
  ...
  initializeClient(): void {
    ...
    this.gapiRef.gapi.client.init(this.setupClientInitData()).then(() => {
  ...
  updateAuthState(): void {
    this.gapiRef.gapi.auth2
      .getAuthInstance()
      .isSignedIn.listen((isSignInResult) => {
        isSignInResult
   ...
   */
  signIn(): any {
    return this.gapiRef.gapi.auth2.getAuthInstance().signIn();
  }
   ...
   */
  signOut(): any {
    return this.gapiRef.gapi.auth2.getAuthInstance().signOut();
    // this.authenticationService.signOut();
  }
